...
func main() {
	...
	if err != nil {
		glog.Fatalf("error building kubeFledged clientset: %s", err.Error())
	}

	old, err := client.KubefledgedV1alpha2().ImageCaches("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		glog.Fatalf("error listing kubeFledged: %s", err.Error())
	}
	...
	for _, o := range old.Items {
		...
		logrus.Info("Migrating kubeFledged: ", o.Name, o.Namespace)
		...
		if err != nil {
			glog.Fatalf("error creating kubeFledged: %s", err.Error())
		}
	}
